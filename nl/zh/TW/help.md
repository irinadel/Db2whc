...
在這裡，您將找到在 {{site.data.keyword.Bluemix_short}} 使用 Db2 Warehouse on Cloud 的常見問題解答。
{: shortdesc}
...
{{site.data.keyword.Bluemix_notm}} 中的 Db2 Warehouse on Cloud 服務可能無法使用。
{: shortdesc}

嘗試建立資料倉儲失敗且錯誤為 500，或 SDP 載入報告 SQL 錯誤。
{: tsSymptoms}

{{site.data.keyword.Bluemix_notm}} 中的 Db2 Warehouse on Cloud 服務正在進行排程的維護程序，或發生已知的中斷。
{: tsCauses}

檢查下列狀態頁面，您就可以判定 {{site.data.keyword.Bluemix_notm}} 中 Db2 Warehouse on Cloud 服務的狀態：
{: tsResolve}

* [{{site.data.keyword.Bluemix_notm}} 支援中心 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://developer.ibm.com/bluemix/support/#status "外部鏈結圖示"){:new_window}
* 狀態監視：
  * [所有地區 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://console.eu-gb.bluemix.net/status?tags=platform,runtimes,services,ibm:yp:eu-gb,ibm:yp:eu-de,ibm:yp:us-south,ibm:yp:au-syd "外部鏈結圖示"){:new_window}
...
如果您使用 Db2 Warehouse on Cloud 時有問題或疑問，可以搜尋資訊或透過討論區提問來取得協助。您也可以開立支援問題單。
{: shortdesc}

使用討論區來發問時，請標記您的問題，讓 {{site.data.keyword.Bluemix}} 開發團隊可以看到它。

* 如果您對於使用 Db2 Warehouse on Cloud 來開發或部署應用程式有技術方面的問題，請將您的問題張貼在 [Stack Overflow ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](http://stackoverflow.com/search?q=dashdb+bluemix "外部鏈結圖示"){:new_window}，並且在您的問題上標記 "bluemix" 和 "db2"。
* 對於服務的相關問題，以及開始使用的指示，請使用 [IBM developerWorks® dW Answers ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://developer.ibm.com/answers/topics/dashdb/?smartspace=bluemix "外部鏈結圖示"){:new_window} 討論區。請包含 "bluemix" 及 "db2" 標籤。
